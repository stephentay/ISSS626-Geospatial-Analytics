...
```
The G-function is near the upper boundary of the simulation envelope. We would need to zoom in to be able to tell whether the G-function has crossed the boundary. If it's within the boundary, then the childcare locations in Tampines exhibit a random pattern. If it exceeds the upper boundary, then it could suggest a clustered pattern.
```{r}
plot(G_tm.csr)
```
:::
